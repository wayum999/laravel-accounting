...
class TransactionCompleteTest extends TestCase
{
    public function test_verify_credits_equal_debits_with_detailed_message(): void
    {
        ...
        $transaction->addTransaction($account1, 'credit', $creditMoney, 'Credit');
        $transaction->addTransaction($account2, 'debit', $debitMoney, 'Debit');
    ...
    public function test_multiple_transactions_verification(): void
    {
        ...
        $transaction->addTransaction($account1, 'credit', new Money(1000, new Currency('USD')), 'Credit 1');
        $transaction->addTransaction($account2, 'credit', new Money(500, new Currency('USD')), 'Credit 2');
        $transaction->addTransaction($account3, 'debit', new Money(1000, new Currency('USD')), 'Debit 1');
    ...
    public function test_mixed_debit_credit_calculations(): void
    {
        ...
        $transaction->addTransaction($account1, 'debit', new Money(800, new Currency('USD')), 'Debit 1');
        $transaction->addTransaction($account2, 'debit', new Money(300, new Currency('USD')), 'Debit 2');
        $transaction->addTransaction($account3, 'credit', new Money(600, new Currency('USD')), 'Credit 1');
        $transaction->addTransaction($account4, 'credit', new Money(400, new Currency('USD')), 'Credit 2');

...
class TransactionExceptionHandlingTest extends TestCase
{
    public function test_commit_exception_handling_with_database_failure(): void
    {
        ...
        DB::statement('DROP TABLE IF EXISTS temp_accounting_accounts');
        DB::statement('ALTER TABLE accounting_accounts RENAME TO temp_accounting_accounts');

        try {
            $transaction->commit();
        } finally {
            // Restore the table for other tests
            DB::statement('ALTER TABLE temp_accounting_accounts RENAME TO accounting_accounts');
        }
    }
    ...
    public function test_transaction_with_multiple_currencies(): void
    {
        ...
        $transaction->addTransaction($account1, 'debit', $usdMoney, 'USD debit');
        $transaction->addTransaction($account2, 'credit', $eurMoney, 'EUR credit');
